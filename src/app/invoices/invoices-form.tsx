...
export default function InvoicesForm() {
  ...
  const [form, setForm] = useState({ supplierId: 0, number: '', date: new Date().toISOString().slice(0,10), vatRate: 0.17 });
      ...
      <div className="card">
        ...
        {lines.map((l, i) => (
          <div key={i} className="grid cols-3" style={{ alignItems: 'end' }}>
            ...
            <label>
              כמות
              <input type="number" step="0.01" value={l.qty} onChange={(e) => setLine(i, { qty: Number(e.target.value) })} />
            </label>
            <label>
              מחיר יחידה
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              {lines.length > 1 && <button type="button" onClick={() => removeLine(i)}>-</button>}
            </div>
          </div>
        ))}
      </div>
